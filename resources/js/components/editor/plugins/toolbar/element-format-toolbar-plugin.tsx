...
import { AlignCenterIcon, AlignJustifyIcon, AlignLeftIcon, AlignRightIcon, IndentDecreaseIcon, IndentIncreaseIcon } from 'lucide-react';
...
export function ElementFormatToolbarPlugin() {
    ...
    return (
        <ToggleGroup type="single" value={elementFormat} defaultValue={elementFormat} onValueChange={handleValueChange} className="flex gap-1">
            {/* Alignment toggles */}
            {Object.entries(ELEMENT_FORMAT_OPTIONS).map(([value, option]) => (
                <ToggleGroupItem key={value} value={value} variant={'outline'} aria-label={option.name} size="sm" className="h-8 w-8 p-0">
                    {option.icon}
                </ToggleGroupItem>
            ))}
